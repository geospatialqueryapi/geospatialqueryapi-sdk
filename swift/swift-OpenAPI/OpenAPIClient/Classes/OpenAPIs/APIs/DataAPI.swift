...
open class DataAPI {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getV1BoundariesUstractLatLatLon(latLon: String, apiResponseQueue: DispatchQueue = OpenAPIClient.apiResponseQueue, completion: @escaping ((_ data: FeatureGeoJSON?, _ error: Error?) -> Void)) {
        getV1BoundariesUstractLatLatLonWithRequestBuilder(latLon: latLon).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<FeatureGeoJSON> 
     */
    open class func getV1BoundariesUstractLatLatLonWithRequestBuilder(latLon: String) -> RequestBuilder<FeatureGeoJSON> {
